...
def main():
    ...
    # gatewayAddr = 'b827ebf74482'.lower()
...
def makeData(sensData, id, dateTime):
    ...
    if not id == '475b41f200350016':
        ...
        # part 5 SCD30 sensor data
        sensDict['CO2'] = sensData[26]
        sensDict['SCD30_temperature'] = sensData[27]
        sensDict['SCD30_humidity'] = sensData[28]
...
def sensorParse(id, dateTime, sensData):
    ...
    if sensData[0] == '4':
        ...
        if sensorDataBuffer['{}_flag'.format(id)] != 4:
            return
        sensorDataBuffer['{}_5'.format(id)] = sensorDataBuffer['{}_4'.format(id)] + sensData[1:]
        sensorDataBuffer['{}_flag'.format(id)] = 5

        # Merge dataframe and print
        sensorDataFrame = sensorDataBuffer['{}_5'.format(id)]
        print(sensorDataFrame)
        sensorDataFrame = makeData(sensorDataFrame, id, sensorDataBuffer['{}_dateTime'.format(id)])
        print('Node {} packet received at {}.'.format(id, sensorDataBuffer['{}_dateTime'.format(id)]))
        try:
...
def write2csv(row):
    # fileDirectory = '/home/rxhf/mintsData/raw/{}/{}/'.format(gatewayAddr, row['id'])
    fileDirectory = gatewayDirectory +str(row['id']) + "/"
    ...
    with open(filename, 'ab') as fileOpen:
        if os.stat(filename).st_size == 0:
            csv_columns = ['dateTime', 'id', 'NH3', 'CO', 'NO2', 'C3H8', 'C4H10', 'CH4', 'H2',
                ...
                'CO2', 'SCD30_temperature', 'SCD30_humidity']

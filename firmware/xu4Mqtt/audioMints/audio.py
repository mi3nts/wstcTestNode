...
def openAudioFile(audio, sample_rate=48000, offset=0.0, duration=None):    
    
    # Open file with librosa (uses ffmpeg or libav)
    import librosa

    try:
        sig, rate = librosa.load(audio, sr=sample_rate, offset=offset, duration=duration, mono=True, res_type='kaiser_fast')
    except:
